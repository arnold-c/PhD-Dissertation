...
In Chapter 3, I map the association between these emergent risk groups with infection rates from serological data to parameterize a mechanistic model of infection, and demonstrate the limits of non-pharmaceutical interventions alone to reduce infections within the student population.
...
$R_"E"$ is the average number of secondary infections each infectious individual is expected to generate before they recover (given the current population size and susceptibility), where values greater than or equal to 1 indicate transmission would be self-sustaining if a population is seeded with infection(s).
